...
import { DevSnapshot } from "./devSnapshot.js";
...
export const Events = () => {
    ...
    Game.onAttack = ({ attacker, row, col, result, turn }) => {
        ...
        if (attacker === 'computer') {
            Storage.captureComputerState(Game.computer.targetQueue, Game.computer.activeHunts)
        }
    }
    ...
    function handleReload() {
        ...
        if (STAGE === "placement" || STAGE === "allPlaced") {
            ...
            if (ships.length > 0) {
                ships.forEach(ship => {
                    Game.player.board.placeShip(ship.row, ship.col, ship.length, ship.dir, ship.id)
        ...
        if(STAGE === "game") {
            ...
            compShips.forEach(ship => {
                Game.computer.board.placeShip(ship.row, ship.col, ship.length, ship.axis, ship.id)
            })
    ...
    function applyAttackRender(cell, result, field, row, col) {
        ...
        if (result === 'sunk' || result === 'allSunk') {
            ...
            Render.renderSink(firstCell, cellNode.ship.length, cellNode.ship.axis, cellNode.ship.id, field)
        }
    }
    ...
    Elements.gameArea.playerContField.addEventListener("click", (e) => {
        ...
        if(cell) {
            ...
            if (isValid(row, col)) {
                ...
                Game.player.board.placeShip(row, col, vars.activeShipLength, vars.placeDir, vars.activeShipName)
                const data = {
                    row: row,
                    col: col,
    ...
    Elements.gameArea.enemyContField.addEventListener("click", (e) => {
        ...
        const cell = e.target.closest(".cell")
        if (cell) {
            const attack = Game.handleAttack("player", cell.dataset.row, cell.dataset.col)
            ...
            if(attack === 'hit') {
                ...
                Render.renderHit(cell)
                // render sinking:
                const firstCell = Game.computer.board.getFirstShipCell(cell.dataset.row, cell.dataset.col)
                const cellNode = Game.computer.board.getCell(cell.dataset.row, cell.dataset.col)
                Render.renderSink(firstCell, cellNode.ship.length, cellNode.ship.axis, cellNode.ship.id, 'computer')
