...
export class Computer extends Player {
	...
	attackHumanWaters(humanBoard) {
		...
		const handleHit = (humanBoard, row, col) => {
			try {
						...
						if (hunt.hits.length > 1) {
							...
							if (hunt.axis === "x") {
								this.targetQueue = this.targetQueue.filter(
									(t) => t.ship !== ship || t.row === r1,
								);
							} else if (hunt.axis === "y") {
								this.targetQueue = this.targetQueue.filter(
									(t) => t.ship !== ship || t.col === c1,
								);
							}
				...
				if (error) {
					if (
						error.message === "This cell is already hit" &&
	...
	_queueNeighbors(row, col, axis, ship, humanBoard) {
		...
		this.targetQueue.push(
			...neighbors.map(([r, c]) => ({ row: r, col: c, ship })),
		);
	}
	...
	_getNeighbors(row, col, axis, humanBoard) {
		...
		// filter by bounds:
		let filtered = neighbors.filter(
			(coord) => coord[0] < 10 && coord[0] >= 0 && coord[1] < 10 && coord[1] >= 0,
		);
	...
	restoreState(serializedState, humanBoard) {
		...
		this.targetQueue = serializedState.targetQueue.map(
			({ row, col, shipId }) => ({
				row,
				col,
