...
test("Place ship horizontally", () => {
    ...
    expect(board.board[1][2]).toEqual({"isHit": false, "ship": {"hits": 0, "length": 3, "sunk": false}})
    expect(board.board[1][3]).toEqual({"isHit": false, "ship": {"hits": 0, "length": 3, "sunk": false}})
    expect(board.board[1][4]).toEqual({"isHit": false, "ship": {"hits": 0, "length": 3, "sunk": false}})
})

test("Place ship vertically", () => {
    const board = new Gameboard()
    board.placeShip(2, 3, 2, "y")
    expect(board.board[1][2]).toEqual({"isHit": false, "ship": {"hits": 0, "length": 2, "sunk": false}})
    expect(board.board[2][2]).toEqual({"isHit": false, "ship": {"hits": 0, "length": 2, "sunk": false}})
})

test("A ship is out of bond", () => {
    const board = new Gameboard()
    expect(() => board.placeShip(1, 8, 4, "x")).toThrow("Coordinates are out of bond")
})

test("A cell is already occupied", () => {
    const board = new Gameboard()
    board.placeShip(2, 2, 2, 'x')
    expect(() => board.placeShip(1, 2, 4, "y")).toThrow("This cell is already occupied")
})
...
test("Receive a hit attack", () => {
    ...
    expect(board.board[1][1]).toEqual({"isHit": true, "ship": {"hits": 1, "length": 2, "sunk": false}})
})
