...
export let Game = {
	...
	handleAttack(attacker, row = null, col = null) {
		...
		if (attacker === "player") {
			...
			if (attack === "hit") {
				setTimeout(() => {
					Render.renderHit(targetCell);
				}, 1000);
				setTimeout(
					() => Render.updateStatusBar("hitEnemy", this.player.name),
					1000,
				);
				setTimeout(() => this.handleAttack("computer"), 2000);
			} else if (attack === "sunk") {
				const firstCell = this.player.board.getFirstShipCell(
					targetCell.dataset.row,
					targetCell.dataset.col,
				);
				const cellNode = this.player.board.getCell(
					targetCell.dataset.row,
					targetCell.dataset.col,
				);
				...
				setTimeout(
					() => Render.updateStatusBar("sinkEnemy", this.player.name),
					1000,
				);
				setTimeout(() => {
					Render.renderSink(
